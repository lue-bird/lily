...
fn initialize_projects_state_for_workspace_directories_into(
    ...
    for project_path in
        list_still_projects_in_directory_path(workspace_directory_paths)
    {
        ...
        StillSyntaxSymbol::ProjectMemberDeclarationName {
            ...
            let origin_declaration_info_markdown: String = match &declaration_node.value {
                ...
                StillSyntaxDeclaration::Variable {
                    ...
                    signature: origin_project_declaration_maybe_signature,
                    parameters: _,
                    equals_key_symbol_range: _,
                    result: _,
...
fn let_declaration_info_markdown(
    ...
    match maybe_signature_type {
        ...
        Some(hovered_local_binding_signature) => {
            let signature_type_internal_comments = still_syntax_comments_in_range(
...
fn state_iter_all_projects<'a>(
    ...
    state.projects.iter().map(|(path,state)| ProjectProjectOriginAndState {
        project_path: path,project_state: state
    })
}
...
fn respond_to_references(
    ...
    Some(match symbol_to_find_node.value {
        ...
        StillSyntaxSymbol::ProjectMemberDeclarationName {
            ...
            all_uses_of_found_project_member.into_iter().map(
                move |use_range_of_found_project| lsp_types::Location {
                    uri: references_arguments
                        .text_document_position
                        .text_document
                        .uri.clone(),
    ...
    let description: String = match maybe_signature_type {
        Some(signature_type_node) => {
            let type_internal_comments =
                still_syntax_comments_in_range(comments, signature_type_node.range);
            format!(
                "```still\n{} :{}{}\n```\n",
                start_name_node.value,
                match still_syntax_range_line_span(
...
fn present_port_declaration_info_markdown(
    ...
    let maybe_fully_qualified_name: Option<StillSyntaxNode<String>> = maybe_name.map(|name_node| {
        still_syntax_node_map(name_node, str::to_string)
    });
                ...
                StillSyntaxExpression::LetIn {
                    ...
                    let_declarations
                        ...
                        .find_map(|(previous_declaration_node, current_declaration_node)| {
                            if let StillSyntaxLetDeclaration::VariableDeclaration {
                                ...
                                && let StillSyntaxLetDeclaration::VariableDeclaration {
                                    start_name: previous_declaration_start_name_node,
                                    signature:
                                        Some(StillSyntaxVariableDeclarationSignature {
...
fn list_still_projects_in_directory_path(
    ...
    for path in paths {
        list_files_passing_test_in_directory_at_path_into(&mut result, path.clone(), |path| path.extension().is_some_and(|ext| ext == "still"));
    }
    result
}
...
enum StillSyntaxPattern {
    ...
    ListExact(Vec<StillSyntaxNode<StillSyntaxPattern>>),
...
enum StillSyntaxLetDeclaration {
    ...
    VariableDeclaration {
        ...
        signature: Option<StillSyntaxVariableDeclarationSignature>,
...
enum StillSyntaxExpression {
    Unit,
    Call {
        called: StillSyntaxNode<Box<StillSyntaxExpression>>,
        argument0: StillSyntaxNode<Box<StillSyntaxExpression>>,
        argument1_up: Vec<StillSyntaxNode<StillSyntaxExpression>>,
    },
    CaseOf {
    ...
    OperatorFunction(StillSyntaxNode<&'static str>),
    Parenthesized(StillSyntaxNode<Box<StillSyntaxExpression>>),
...
fn still_syntax_type_to_unparenthesized(
    ...
    match still_syntax_type.value {
        StillSyntaxType::Parenthesized(in_parens) => {
    ...
    match type_node.value {
        ...
        StillSyntaxType::Unit => {
            ...
            if comments_in_unit.is_empty() {
                ...
                so_far.push('(');
                still_syntax_comments_into(so_far, indent + 1, comments_in_unit);
                linebreak_indented_into(so_far, indent);
                so_far.push(')');
            }
        }
...
fn still_syntax_expression_not_parenthesized_into(
    ...
    match expression_node.value {
        StillSyntaxExpression::Unit => {
            let comments_in_unit = still_syntax_comments_in_range(comments, expression_node.range);
            if comments_in_unit.is_empty() {
                so_far.push_str("()");
            } else {
                so_far.push('(');
        ...
        StillSyntaxExpression::Call {
            ...
            let line_span_before_argument0: LineSpan = if comments_before_argument0.is_empty()
                ...
                && still_syntax_expression_line_span(comments, still_syntax_node_unbox(called_node))
                    == LineSpan::Single
                && still_syntax_expression_line_span(
                    comments,
                    still_syntax_node_unbox(argument0_node),
                ) == LineSpan::Single
            {
                LineSpan::Single
            } else {
            ...
            let full_line_span: LineSpan = match line_span_before_argument0 {
                ...
                LineSpan::Single => still_syntax_expression_line_span(comments, expression_node),
            };
            still_syntax_expression_parenthesized_if_space_separated_into(
                so_far,
                indent,
                comments,
                still_syntax_node_unbox(called_node),
            );
            space_or_linebreak_indented_into(
                so_far,
        ...
        StillSyntaxExpression::OperatorFunction(operator_node) => {
            so_far.push('(');
            so_far.push_str(operator_node.value);
            so_far.push(')');
        }
        StillSyntaxExpression::Parenthesized(in_parens) => {
...
fn still_syntax_variable_declaration_into(
    ...
    comments: &[StillSyntaxNode<StillSyntaxComment>],
    start_name_node: StillSyntaxNode<&str>,
    maybe_signature: Option<&StillSyntaxVariableDeclarationSignature>,
    parameters: &[StillSyntaxNode<StillSyntaxPattern>],
...
fn still_syntax_expression_parenthesized_if_space_separated_into(
    ...
    let is_space_separated: bool = match unparenthesized.value {
        ...
        StillSyntaxExpression::CaseOf { .. } => true,
        StillSyntaxExpression::Unit => false,
        ...
        StillSyntaxExpression::OperatorFunction(_) => false,
...
fn still_syntax_expression_any_sub(
    ...
    match expression_node.value {
        ...
        StillSyntaxExpression::Call {
            ...
            still_syntax_expression_any_sub(still_syntax_node_unbox(called), is_needle)
                || still_syntax_expression_any_sub(still_syntax_node_unbox(argument0), is_needle)
        ...
        match still_syntax_declaration_node.value {
            ...
            StillSyntaxDeclaration::Variable {
                ...
                if lsp_range_includes_position(start_name_node.range, position) {
                    ...
                    maybe_signature
                        .as_ref()
                        .and_then(|signature: &StillSyntaxVariableDeclarationSignature| {
                                ...
                                && lsp_range_includes_position(implementation_name_range, position)
                            {
                                ...
                                signature.type_.as_ref().and_then(|signature_type_node| {
                                    still_syntax_type_find_reference_at_position(
                                        still_syntax_declaration_node.value,
                                        still_syntax_node_as_ref(signature_type_node),
                                        position,
                                    )
                                })
                            }
                        ...
                        .or_else(|| {
                            let mut parameter_introduced_bindings: Vec<StillLocalBinding> =
                                Vec::new();
                            for parameter_node in parameters {
...
fn still_syntax_pattern_find_reference_at_position<'a>(
    ...
    match still_syntax_pattern_node.value {
        ...
        StillSyntaxPattern::ListExact(elements) => elements.iter().find_map(|element| {
            still_syntax_pattern_find_reference_at_position(
...
enum LocalBindingOrigin<'a> {
    ...
    LetDeclaredVariable {
        signature: Option<&'a StillSyntaxVariableDeclarationSignature>,
        start_name_range: lsp_types::Range,
    },
}
#[derive(Clone, Copy, Debug)]
    ...
    match still_syntax_expression_node.value {
        StillSyntaxExpression::Call {
            ...
            local_bindings = still_syntax_expression_find_reference_at_position(
                local_bindings,
                scope_declaration,
                still_syntax_node_unbox(called),
                position,
            )?;
            local_bindings = still_syntax_expression_find_reference_at_position(
                local_bindings,
                scope_declaration,
                still_syntax_node_unbox(argument0),
                position,
            )?;
            argument1_up
                .iter()
        ...
        StillSyntaxLetDeclaration::VariableDeclaration {
            ...
            if let Some(signature) = maybe_signature {
                    ...
                    && lsp_range_includes_position(implementation_name_range, position)
                {
                    return std::ops::ControlFlow::Break(StillSyntaxNode {
                        value: StillSyntaxSymbol::LetDeclarationName {
                            name: &start_name.value,
                            start_name_range: start_name.range,
                            signature_type: signature.type_.as_ref().map(still_syntax_node_as_ref),
                            scope_expression: scope_expression,
                        },
                ...
                if let Some(signature_type_node) = &signature.type_ {
                    on_some_break(still_syntax_type_find_reference_at_position(
                        scope_declaration,
                        still_syntax_node_as_ref(signature_type_node),
                        position,
                    ))?;
            ...
            match maybe_result {
                Some(result_node) => {
                    ...
                    local_bindings
                        .push((still_syntax_node_as_ref(result_node), introduced_bindings));
                    still_syntax_expression_find_reference_at_position(
                        local_bindings,
                        scope_declaration,
                        still_syntax_node_as_ref(result_node),
...
fn still_syntax_declaration_uses_of_reference_into(
    ...
    match still_syntax_declaration {
        ...
        StillSyntaxDeclaration::Variable {
            ...
            let mut parameter_bindings: Vec<StillLocalBinding> = Vec::new();
            for parameter_node in parameters {
                still_syntax_pattern_bindings_into(
    ...
    match still_syntax_expression_node.value {
        ...
        StillSyntaxExpression::Reference {
                ...
                && symbol_name == name.as_str()
            {
                if  local_bindings
                        .iter()
                        .any(|local_binding| local_binding.name == name.as_str())
                {
...
fn still_syntax_pattern_bindings_into<'a>(
    ...
    match still_syntax_pattern_node.value {
        ...
        StillSyntaxPattern::Variable(variable) => {
            bindings_so_far.push(StillLocalBinding {
                origin: LocalBindingOrigin::PatternVariable(still_syntax_pattern_node.range),
                name: variable,
            });
        }
...
fn still_syntax_highlight_declaration_into(
    ...
    match still_syntax_declaration_node.value {
        StillSyntaxDeclaration::Variable {
            ...
            if let Some(result_node) = maybe_result {
                let mut local_bindings: Vec<StillLocalBinding> = Vec::new();
                for parameter_node in parameters {
                    still_syntax_pattern_bindings_into(
                        &mut local_bindings,
                        still_syntax_node_as_ref(parameter_node),
    ...
    match still_syntax_expression_node.value {
        ...
        StillSyntaxExpression::Reference {
                    ...
                    .find(|bind| bind.name == name.as_str())
            {
                ...
                highlighted_so_far.push(StillSyntaxNode {
                    range: still_syntax_expression_node.range,
                    value: if name.starts_with(|c: char| c.is_uppercase()) {
...
fn parse_still_operator_followed_by_closing_paren(
    state: &mut ParseState,
) -> Option<StillSyntaxNode<&'static str>> {
    // can be optimized by only slicing once for each symbol length
    let start_position: lsp_types::Position = state.position;
...
fn parse_still_syntax_pattern_space_separated_node_starting_at_any_indent(
    ...
    let start_pattern: StillSyntaxNode<StillSyntaxPattern> =
        parse_still_syntax_pattern_not_as_or_cons_node(state)?;
    parse_still_whitespace_and_comments(state);
        ...
        |(cons_tail_key_symbol_range, maybe_tail), (cons_head_key_symbol_range, maybe_head)| {
            (
                ...
                Some(StillSyntaxNode {
                    ...
                    value: StillSyntaxPattern::ListCons {
...
fn parse_still_syntax_pattern_not_as_or_cons_node(
    ...
    parse_still_syntax_pattern_construct_node(state).or_else(|| {
...
fn parse_still_syntax_pattern_not_space_separated(
    state: &mut ParseState,
...
fn parse_still_syntax_pattern_list_exact(state: &mut ParseState) -> Option<StillSyntaxPattern> {
    ...
    while let Some(pattern_node) = parse_still_syntax_pattern_space_separated_node(state) {
    ...
    let _: bool = parse_symbol(state, "]");
    Some(StillSyntaxPattern::ListExact(elements))
}
...
fn parse_still_syntax_pattern_not_space_separated_node(
    state: &mut ParseState,
) -> Option<StillSyntaxNode<StillSyntaxPattern>> {
    let start_position: lsp_types::Position = state.position;
    parse_still_syntax_pattern_not_space_separated(state).map(|pattern| StillSyntaxNode {
        range: lsp_types::Range {
            start: start_position,
            end: state.position,
        },
...
fn parse_still_syntax_pattern_construct_node(
    state: &mut ParseState,
) -> Option<StillSyntaxNode<StillSyntaxPattern>> {
    let reference_node: StillSyntaxNode<StillName> =
        parse_still_qualified_uppercase_reference_node(state)?;
...
fn parse_still_syntax_expression_call_or_not_space_separated_node(
    ...
    let called_node: StillSyntaxNode<StillSyntaxExpression> =
    ...
    Some(
        if (state.position.character > u32::from(state.indent))
            && let Some(argument0_node) =
                parse_still_syntax_expression_not_space_separated_node(state)
        {
            let mut argument1_up: Vec<StillSyntaxNode<StillSyntaxExpression>> = Vec::new();
            ...
            StillSyntaxNode {
                ...
                value: StillSyntaxExpression::Call {
                    called: still_syntax_node_box(called_node),
                    argument0: still_syntax_node_box(argument0_node),
...
fn parse_still_syntax_expression_not_space_separated_node(
    ...
    let start_position: lsp_types::Position = state.position;
    let start_expression: StillSyntaxExpression =
        parse_symbol_as(state, "()", StillSyntaxExpression::Unit)
...
fn parse_still_syntax_expression_reference(
    ...
    parse_still_lowercase_name(state)
        .map(|name| StillSyntaxExpression::Reference {
            name: name,
        })
...
fn parse_still_syntax_expression_lambda(
    ...
    while let Some(parameter_node) = parse_still_syntax_pattern_not_space_separated_node(state) {
...
fn parse_still_syntax_let_variable_declaration_node(
    ...
    parse_still_whitespace_and_comments(state);
    Some(match parse_symbol_as_range(state, ":") {
        None => parse_still_syntax_let_variable_declaration_node_after_maybe_signature_and_name(
            state,
            start_name_node,
            None,
        ),
            ...
            match parse_symbol_as_range(state, &start_name_node.value) {
                None => StillSyntaxNode {
                    ...
                    value: StillSyntaxLetDeclaration::VariableDeclaration {
                        start_name: start_name_node,
                        signature: Some(StillSyntaxVariableDeclarationSignature {
                ...
                Some(implementation_name_range) => {
                    parse_still_whitespace_and_comments(state);
                    parse_still_syntax_let_variable_declaration_node_after_maybe_signature_and_name(
                        state,
                        start_name_node,
                        Some(StillSyntaxVariableDeclarationSignature {
                            colon_key_symbol_range: colon_key_symbol_range,
...
fn parse_still_syntax_let_variable_declaration_node_after_maybe_signature_and_name(
    state: &mut ParseState,
    start_name_node: StillSyntaxNode<StillName>,
    maybe_signature: Option<StillSyntaxVariableDeclarationSignature>,
) -> StillSyntaxNode<StillSyntaxLetDeclaration> {
...
fn parse_still_syntax_expression_operator_function_or_parenthesized(
    ...
    parse_still_whitespace_and_comments(state);
    Some(
        if let Some(operator_node) = parse_still_operator_followed_by_closing_paren(state) {
...
fn parse_still_syntax_declaration_variable_node(
    ...
    parse_still_whitespace_and_comments(state);
    Some(match parse_symbol_as_range(state, ":") {
        None => parse_still_syntax_declaration_variable_node_after_maybe_signature_and_name(
            state,
            start_name_node,
            None,
        ),
        Some(colon_key_symbol_range) => {
            ...
            match parse_symbol_as_range(state, &start_name_node.value) {
                ...
                Some(implementation_name_range) => {
                    parse_still_whitespace_and_comments(state);
                    parse_still_syntax_declaration_variable_node_after_maybe_signature_and_name(
                        state,
                        start_name_node,
                        Some(StillSyntaxVariableDeclarationSignature {
                            colon_key_symbol_range: colon_key_symbol_range,
...
fn parse_still_syntax_declaration_variable_node_after_maybe_signature_and_name(
    state: &mut ParseState,
    start_name_node: StillSyntaxNode<StillName>,
    maybe_signature: Option<StillSyntaxVariableDeclarationSignature>,
) -> StillSyntaxNode<StillSyntaxDeclaration> {
    let mut syntax_before_equals_key_symbol_end_location: lsp_types::Position =
        match &maybe_signature {
            Some(signature) => signature
                .implementation_name_range
