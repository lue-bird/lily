                ...
                hovered_project_state.choice_types.get(hovered_name)
            {
                present_choice_type_declaration_info_markdown(
                    ...
                    origin_choice_type_info
                        ...
                        .as_ref()
                        .map(still_syntax_node_as_ref),
                    &origin_choice_type_info.variant1_up,
                )
...
fn still_syntax_expression_type_with<'a>(
    ...
    match expression_node.value {
        ...
        StillSyntaxExpression::VariableOrCall {
            ...
            None => match variable_declarations.get(variable_node.value.as_str()) {
                None => StillSyntaxNode {
                    range: expression_node.range,
                    value: StillSyntaxType::Parenthesized(None),
                },
                Some(maybe_project_variable_info) => {
                    let Some(project_variable_type_node) = &maybe_project_variable_info.type_
                    else {
                    ...
                    if arguments.is_empty() {
                        ...
                        let Some((inputs, maybe_variable_type_output)) =
                            still_syntax_type_to_function(
                                type_aliases,
...
const still_syntax_type_chr: StillSyntaxType = StillSyntaxType::Construct {
    name: still_syntax_node_empty(StillName::const_new("chr")),
    arguments: vec![],
};
    ...
    match expression_node.value {
        ...
        StillSyntaxExpression::CaseOf {
            ...
            if let Some((case0, case1_up)) = cases.split_first() {
                still_syntax_case_into(so_far, next_indent(indent), case0);
                for case in case1_up {
                    linebreak_indented_into(so_far, next_indent(indent));
                    still_syntax_case_into(so_far, next_indent(indent), case);
                }
            }
        }
        ...
        StillSyntaxExpression::CaseOf {
            ...
            cases
                ...
                .try_fold(local_bindings, |mut local_bindings, case| {
                    if let Some(found_symbol) = still_syntax_pattern_find_symbol_at_position(
                        scope_declaration,
...
enum StillSyntaxTypeDeclarationInfo<'a> {
    ...
    ChoiceType {
        ...
        variant1_up: &'a Vec<StillSyntaxChoiceTypeDeclarationTailingVariant>,
    },
    TypeAlias {
        range: lsp_types::Range,
        documentation: &'a Option<StillSyntaxNode<Box<str>>>,
            ...
            match type_declaration_info {
                ...
                StillSyntaxTypeDeclarationInfo::ChoiceType {
                    ...
                    let compiled_choice_type_info: CompiledRustChoiceTypeInfo =
                        choice_type_declaration_to_rust_into(
                            ...
                            maybe_variant0_name
                                .as_ref()
                                .map(|n| still_syntax_node_as_ref_map(n, StillName::as_str)),
                            maybe_variant0_value.as_ref().map(still_syntax_node_as_ref),
                            variant1_up,
                        );
                    let info: ChoiceTypeInfo = ChoiceTypeInfo {
                        ...
                        variant0_value: maybe_variant0_value.clone(),
                        variant1_up: variant1_up.clone(),
    ...
    for variable_declaration_strongly_connected_component in variable_graph.find_sccs().iter_sccs()
    {
        ...
        for variable_declaration in &variable_declarations_in_strongly_connected_component {
            match variable_declaration.result {
                ...
                Some(result_node) => {
                    match still_syntax_expression_type(
                        &compiled_type_alias_infos,
                        &compiled_variable_declaration_infos,
                        result_node,
                    ) {
                        // is there a better way? Maybe do complain when used variables have no known type
                        StillSyntaxNode {
                            range: _,
                            value: StillSyntaxType::Parenthesized(None),
                        } => {
...
-> std::collections::HashMap<StillName, CompiledVariableDeclarationInfo> {
    ...
    fn function(
        inputs: impl IntoIterator<Item = StillSyntaxType>,
        output: StillSyntaxType,
    ) -> StillSyntaxType {
        StillSyntaxType::Function {
            inputs: inputs
                .into_iter()
                .map(still_syntax_node_empty)
                .collect::<Vec<_>>(),
            arrow_key_symbol_range: None,
            output: Some(still_syntax_node_empty(Box::new(output))),
        }
    }
    fn vec(element: StillSyntaxType) -> StillSyntaxType {
        still_syntax_type_vec(still_syntax_node_empty(element))
    }
    fn opt(value: StillSyntaxType) -> StillSyntaxType {
        StillSyntaxType::Construct {
            name: still_syntax_node_empty(StillName::const_new("opt")),
            arguments: vec![still_syntax_node_empty(value)],
        }
    }
...
"#
        ...
        (
            ...
            ChoiceTypeInfo {
                ...
                variant0_name: Some(still_syntax_node_empty(StillName::from("Absent"))),
                variant0_value: None,
                variant1_up: vec![StillSyntaxChoiceTypeDeclarationTailingVariant {
...
fn choice_type_declaration_to_rust_into<'a>(
    ...
    maybe_variant0_name: Option<StillSyntaxNode<&'a str>>,
    ...
    for (variant_index, (maybe_variant_name, variant_error_range, maybe_variant_value)) in
        std::iter::once((maybe_variant0_name, range, maybe_variant0_value))
            .chain(variant1_up.iter().map(|variant| {
                (
                    variant
                        .name
                        .as_ref()
            ...
            .enumerate()
    {
        match maybe_variant_name {
            None => {
                // no point in generating a variant since it's never referenced
                errors.push(StillErrorNode {
                    range: variant_error_range,
                    message: Box::from("missing variant name"),
                });
            }
            Some(variant_name) => {
                let variant_value_constructs_recursive_type: bool = maybe_variant_value
                    .is_some_and(|variant_value_node| {
                ...
                rust_variants.push(still_syntax_variant_to_rust(
                    |variant_value_node| {
                        let rust_variant_value: syn::Type = still_syntax_type_to_rust(
                            errors,
                            records_used,
                            type_aliases,
                            choice_types,
                ...
                if variant_value_constructs_recursive_type {
                    recursive_variant_value_variant_indexes.push(variant_index);
                }
            }
        }
    }
    let is_copy: bool = std::iter::once(maybe_variant0_value)
        .chain(
            variant1_up
                .iter()
                .map(|variant| variant.value.as_ref().map(still_syntax_node_as_ref)),
        )
        .enumerate()
        ...
        .all(|(variant_index, variant_value_node)| {
            recursive_variant_value_variant_indexes.contains(&variant_index)
                || still_syntax_type_is_copy(true, type_aliases, choice_types, variant_value_node)
        });
    let is_recursive: bool = !recursive_variant_value_variant_indexes.is_empty();
    let has_lifetime_parameter: bool = is_recursive
        || std::iter::once(maybe_variant0_value)
            .chain(
                variant1_up
                    .iter()
                    .map(|variant| variant.value.as_ref().map(still_syntax_node_as_ref)),
    ...
    if is_recursive {
        // TODO generate even for non-recursive
        let owned_rust_enum_name: String = still_name_to_uppercase_rust(name) + "øOwned";
        let mut owned_rust_variants: syn::punctuated::Punctuated<syn::Variant, syn::token::Comma> =
            syn::punctuated::Punctuated::new();
        for (variant_index, (maybe_variant_name, maybe_variant_value)) in
            std::iter::once((maybe_variant0_name, maybe_variant0_value))
                .chain(variant1_up.iter().map(|variant| {
                    (
                        variant
                            .name
                ...
                .enumerate()
        {
            if let Some(variant_name) = maybe_variant_name {
                owned_rust_variants.push(still_syntax_variant_to_rust(
                    |variant_value_node| {
                        let rust_variant_value: syn::Type = still_syntax_type_to_rust(
                            errors,
                            records_used,
                        ...
                        let owned_rust_variant_value: syn::Type = syn::Type::Path(syn::TypePath {
                            qself: Some(syn::QSelf {
                                lt_token: syn::token::Lt(syn_span()),
                                ty: Box::new(rust_variant_value),
                                position: 1,
        ...
        let impl_still_to_owned: syn::Item = syn::Item::Impl(syn::ItemImpl {
            ...
            generics: syn::Generics {
                lt_token: Some(syn::token::Lt(syn_span())),
                params: std::iter::once(syn::GenericParam::Lifetime(syn::LifetimeParam::new(
                    syn_default_lifetime(),
                )))
                .chain(parameters.iter().map(|parameter_node| {
                    syn::GenericParam::Type(syn::TypeParam {
            ...
            self_ty: Box::new(syn::Type::Path(syn::TypePath {
                ...
                path: syn_path_name_with_arguments(
                    &rust_enum_name,
                    std::iter::once(syn::GenericArgument::Lifetime(syn_default_lifetime())).chain(
                        parameters.iter().map(|parameter_node| {
                            syn::GenericArgument::Type(syn_type_variable(
                                &still_type_variable_to_rust(&parameter_node.value),
                            ))
                        }),
                    ),
                    ...
                    block: syn::Block {
                        ...
                        stmts: vec![syn::Stmt::Expr(
                            syn::Expr::Match(syn::ExprMatch {
                                ...
                                arms: maybe_variant0_name
                                    .map(|name_node| (name_node, maybe_variant0_value))
                                    .into_iter()
                                    .chain(variant1_up.iter().filter_map(|variant| {
                    ...
                    predicates: parameters
                        ...
                        .map(|parameter_node| {
                            syn::WherePredicate::Type(syn::PredicateType {
                                ...
                                colon_token: syn::token::Colon(syn_span()),
                                bounds: std::iter::once(syn::TypeParamBound::Trait(
                                    syn::TraitBound {
                                        paren_token: None,
                                        modifier: syn::TraitBoundModifier::None,
                                        lifetimes: None,
            ...
            items: vec![
                syn::ImplItem::Type(syn::ImplItemType {
                    ...
                    ty: syn::Type::Path(syn::TypePath {
                        ...
                        path: syn_path_name_with_arguments(
                            &rust_enum_name,
                            std::iter::once(syn::GenericArgument::Lifetime(syn_default_lifetime()))
                                .chain(parameters.iter().map(|parameter_node| {
                                    syn::GenericArgument::Type(syn::Type::Path(syn::TypePath {
                                        qself: Some(syn::QSelf {
                                            lt_token: syn::token::Lt(syn_span()),
                                        ...
                                        syn::Arm {
                                            ...
                                            body: Box::new(match maybe_variant_value {
                                                ...
                                                Some(_) => syn::Expr::Call(syn::ExprCall {
                                                    ...
                                                    func: Box::new(rust_result_variant_constructor),
                                                    paren_token: syn::token::Paren(syn_span()),
                                                    args: std::iter::once( syn::Expr::Call(syn::ExprCall {
                                                            attrs: vec![],
                                                            func: Box::new(syn_expr_reference([
                                                                "OwnedToStill",
                                                                "to_still",
    ...
    CompiledRustChoiceTypeInfo {
        is_copy: is_copy,
        has_lifetime_parameter: has_lifetime_parameter,
        recursive_variant_value_variant_indexes: recursive_variant_value_variant_indexes,
    }
}
fn still_syntax_variant_to_rust<'a>(
    variant_value_to_rust_type: impl FnOnce(StillSyntaxNode<&'a StillSyntaxType>) -> syn::Type,
    ...
    syn::Variant {
        ...
        fields: match variant_value {
            ...
            Some(variant_value_node) => syn::Fields::Unnamed(syn::FieldsUnnamed {
                paren_token: syn::token::Paren(syn_span()),
                unnamed: std::iter::once(syn::Field {
                    ...
                    ident: None,
                    colon_token: None,
                    ty: variant_value_to_rust_type(variant_value_node),
                })
...
fn variable_declaration_to_rust<'a>(
    ...
    let (rust_type, maybe_still_type_node) = match variable_declaration_info.result {
        ...
        Some(result_node) => {
            let type_node = still_syntax_expression_type_with(
                type_aliases,
                variable_declarations,
                std::rc::Rc::new(std::collections::HashMap::new()),
                result_node,
            ...
            (
                still_syntax_type_to_rust(
                    // errors are reported when compiling expression
                    &mut Vec::new(),
                    // TODO this is dumb.
    ...
    {
        None => {
            let compiled_result: CompiledStillExpression = match variable_declaration_info.result {
            ...
            .and_then(still_syntax_expression_to_lambda)
        {
                ...
                (
                    syn::Item::Fn(syn::ItemFn {
                        ...
                        sig: syn::Signature {
                            ...
                            output: syn::ReturnType::Type(
                                syn::token::RArrow(syn_span()),
                                Box::new(match maybe_output {
                                    None => syn_type_infer(),
                                    Some(output_node) => still_syntax_type_to_rust(
                                        // errors are reported when compiling expression
                                        &mut Vec::new(),
                                        // TODO this is dumb.
                                        // consider separating out to_rust from collecting records
                                        ...
                                        choice_types,
                                        syn_default_lifetime_name,
                                        still_syntax_node_as_ref(&output_node),
                                    ),
                                }),
                            ),
                            variadic: None,
            ...
            None => {
                let compiled_result: CompiledStillExpression =
                        ...
                        block: Box::new(syn::Block {
                            ...
                            stmts: vec![syn::Stmt::Expr(
                                syn::Expr::Call(syn::ExprCall {
                                    attrs: vec![],
                                    func: Box::new(compiled_result.rust),
                                    paren_token: syn::token::Paren(syn_span()),
                                    args: inputs
                                        .iter()
                                        .enumerate()
...
fn still_syntax_expression_to_lambda(
    still_type_node: StillSyntaxNode<&StillSyntaxExpression>,
) -> Option<(
    &[StillSyntaxNode<StillSyntaxPattern>],
    Option<StillSyntaxNode<&StillSyntaxExpression>>,
)> {
    match still_type_node.value {
        StillSyntaxExpression::Lambda {
        ...
        StillSyntaxExpression::WithComment {
            comment: _,
            expression: Some(after_comment_node),
        } => still_syntax_expression_to_lambda(still_syntax_node_unbox(after_comment_node)),
        StillSyntaxExpression::Typed {
            type_: _,
            expression:
                Some(StillSyntaxNode {
                    range: in_typed_range,
                    value: StillSyntaxExpressionUntyped::Other(in_typed),
                }),
        } => still_syntax_expression_to_lambda(StillSyntaxNode {
            range: *in_typed_range,
            value: in_typed,
        }),
        StillSyntaxExpression::Parenthesized(Some(in_parens_node)) => {
            still_syntax_expression_to_lambda(still_syntax_node_unbox(in_parens_node))
        }
        _ => None,
    }
}
fn maybe_still_syntax_type_to_rust<'a>(
    ...
    match maybe_type {
        ...
        Some(type_node) => still_syntax_type_to_rust(
            errors,
            records_used,
            type_aliases,
            choice_types,
...
/// TODO consider merging with is-copy and uses-lifetime
fn still_syntax_type_to_rust<'a>(
    ...
    match type_node.value {
        ...
        StillSyntaxType::WithComment {
            ...
            || StillErrorNode {
                range: type_node.range,
                message: Box::from("missing type after the comment # ...\\n here"),
            },
            records_used,
            type_aliases,
            choice_types,
...
fn still_syntax_expression_to_rust<'a>(
    ...
    match expression_node.value {
        ...
        StillSyntaxExpression::Char(maybe_char) => match *maybe_char {
            ...
            Some(char) => CompiledStillExpression {
                uses_allocator: false,
                rust: syn::Expr::Lit(syn::ExprLit {
                    attrs: vec![],
        ...
        StillSyntaxExpression::Dec(dec_or_err) => match dec_or_err.parse::<f64>() {
            ...
            Ok(dec) => CompiledStillExpression {
                uses_allocator: false,
                rust: syn::Expr::Lit(syn::ExprLit {
                    attrs: vec![],
        ...
        StillSyntaxExpression::Int(representation) => match representation.parse::<isize>() {
            ...
            Ok(int) => CompiledStillExpression {
                uses_allocator: false,
                rust: syn::Expr::Lit(syn::ExprLit {
                    attrs: vec![],
                ...
                Some(compiled_origin_variable_declaration) => {
                    ...
                    CompiledStillExpression {
                        rust: syn::Expr::Call(syn::ExprCall {
                            ...
                            paren_token: syn::token::Paren(syn_span()),
                            args: if compiled_origin_variable_declaration.has_allocator_parameter {
                        ...
                        uses_allocator: uses_allocator
                            || compiled_origin_variable_declaration.has_allocator_parameter,
                    }
                }
                None => {
        ...
        StillSyntaxExpression::Record(fields) => {
            ...
            let rust_fields: syn::punctuated::Punctuated<syn::FieldValue, syn::token::Comma> =
                fields
                    .iter()
                    .map(|field| {
        ...
        StillSyntaxExpression::RecordAccess {
            ...
            match maybe_field_name {
                None => compiled_record,
                Some(field_name_node) => CompiledStillExpression {
    ...
    match &pattern_node.value {
        StillSyntaxPattern::Char(maybe_char) => match *maybe_char {
            None => {
                ...
                // return some random, unlikely character literal that is highly unlikely to exist.
